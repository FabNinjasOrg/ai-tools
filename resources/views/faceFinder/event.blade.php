            ...
            <div x-show="activeTab === 'albums'" class="p-6">
                <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="p-5">
                        ...
                        <div x-show="!albumsLoading && albums.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            <template x-for="album in albums" :key="album.id">
                                <a :href="'{{ route('face_finder.albums.show', ['id' => 'ALBUM_ID']) }}'.replace('ALBUM_ID', album.id)" class="rounded-xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition p-4 block">
                                    <div class="flex items-center gap-3">
                                        <div class="h-10 w-10 rounded-lg bg-purple-100 text-purple-700 inline-flex items-center justify-center">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 7h4l2-2h6l2 2h4v10a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" />
                                            </svg>
                                        </div>
                                        <div class="min-w-0 flex-1">
                                            <div class="text-sm font-semibold text-slate-900 truncate" x-text="album.name"></div>
                                            <div class="text-xs text-slate-500" x-text="album.created_at ? new Date(album.created_at).toLocaleDateString() : ''"></div>
                                        </div>
                                    </div>
                                </a>
                            </template>
